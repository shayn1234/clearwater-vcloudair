...
#      # PATCH - start
#      if node[:cloud][:local_ipv4]
#        node.automatic[:cloud][:local_ipv4] = node[:cloud][:local_ipv4].split(',')[0]
#      end
#      # PATCH - end
#      node.save
#    end
    params= File.read("/home/ubuntu/forohai").split
    node.automatic[:cloud][:public_hostname]=params[0]
    node.automatic[:cloud][:local_ipv4]=params[1]
    node.automatic[:cloud][:public_ipv4]=params[1]
    node.automatic[:cloud][:provider]="vcloudair"
    node.save
